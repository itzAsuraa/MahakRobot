...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        update.effective_message.reply_text(f"❖ ᴄᴜʀʀᴇɴᴛ sᴛᴀᴛᴇ ᴏғ ᴀᴠɪsʜᴀ ❖ {MahakRobot.ALLOW_CHATS}")
        return
    if args[0].lower() in ["off", "no"]:
        MahakRobot.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        MahakRobot.ALLOW_CHATS = False
    else:
    ...
    if args:
        ...
        try:
            ...
            update.effective_message.reply_text(
                "⬤ ɪ ᴄᴏᴜʟᴅ ɴᴏᴛ ʟᴇᴀᴠᴇ ᴛʜᴀᴛ ɢʀᴏᴜᴘ(ᴅᴜɴɴᴏ ᴡʜʏ ᴛʜᴏ)."
            )
            return
        with suppress(Unauthorized):
            update.effective_message.reply_text("⬤ ɪ ʟᴇғᴛ ᴛʜᴀᴛ sᴏᴜᴘ !.")
    else:
