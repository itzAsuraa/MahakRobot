...
async def upvote(_, message):
    ...
    if message.reply_to_message.from_user.id == OWNER_ID:
        await message.reply_text(
            "⬤ ʜᴇ's ᴍʏ ᴏᴡɴᴇʀ. sᴏ ʏᴇᴀʜ, ʜᴇ ɪs ᴀʟᴡᴀʏs ʀɪɢʜᴛ ᴀɴᴅ ᴇᴠᴇʀʏᴏɴᴇ ᴋɴᴏᴡs ʜᴇ ɪs ᴀ ɢᴏᴏᴅ ᴘᴇʀsᴏɴ ᴛᴏᴏ."
        )
        return
    ...
    await message.reply_text(
        f"❖ ɪɴᴄʀᴇᴍᴇɴᴛᴇᴅ ᴋᴀʀᴍᴀ ᴏғ {user_mention} ʙʏ 1\n**● ᴛᴏᴛᴀʟ ᴩᴏɪɴᴛs ➥** {karma}\n\n❖ ᴘᴏᴡᴇʀᴇᴅ ʙʏ ➥ ˹ ᴍᴀʜᴀᴋ ꭙ ʀᴏʙᴏᴛ™ ♡゙", reply_markup=InlineKeyboardMarkup(EVAA),
    )
...
async def karma(_, message):
    ...
    if not message.reply_to_message:
        ...
        if not karma:
            ...
            return
        msg = f"**⬤ ᴋᴀʀᴍᴀ ʟɪsᴛ ᴏғ {message.chat.title} ⏤͟͟͞͞★ **\n"
        limit = 0
        karma_dicc = {}
        for i in karma:
