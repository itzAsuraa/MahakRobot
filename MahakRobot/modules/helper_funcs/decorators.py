...
from MahakRobot import dispatcher as n
from MahakRobot.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler
...
class AvishaTelegramHandler:
    ...
    def message(
        ...
        def _message(func):
            try:
                ...
                LOGGER.debug(
                    f"[MahakRobot-Msg] Loaded filter pattern {pattern} for function {func.__name__}"
                )
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            LOGGER.debug(
                f"[MahakRobot-Callback] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}"
            )
            return func
